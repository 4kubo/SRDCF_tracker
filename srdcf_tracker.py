class SRDCF_tracker:
    ...
    def track(self):
        ...
        pos = floor(self.params.init_pos)
        target_sz = floor(self.params.wsize)

        debug = self.params.debug
        visualization = self.params.visualization | | debug
        ...
        featureRatio = self.params.t_global.cell_size

        # h * w * search_area_scale
        search_area = prod(init_target_sz / featureRatio * search_area_scale)

        # when the number of cells are small, choose a
        smaller
        cell
        size

        if isfield(self.params.t_global, 'cell_selection_thresh')
            if search_area < self.params.t_global.cell_selection_thresh * filter_max_area
                self.params.t_global.cell_size = min(featureRatio, max(1, ceil(sqrt(prod(init_target_sz * search_area_scale) / (
                self.params.t_global.cell_selection_thresh * filter_max_area)))))
        ...
        global_feat_params = self.params.t_global
        ...
        account
        switch
        self.params.search_area_shape
        case
        'proportional'
        sz = floor(base_target_sz * search_area_scale) # proportional
        area, same
        aspect
        ratio as the
        target
    ...
    case
    'square'
    sz = repmat(sqrt(prod(base_target_sz * search_area_scale)), 1, 2) # square
    area, ignores
    the
    target
    aspect
    ...
    case
    'fix_padding'
    sz = base_target_sz + sqrt(
        prod(base_target_sz * search_area_scale) + (base_target_sz(1) - base_target_sz(2)) / 4) - sum(
        base_target_sz) / 2 # const
    padding
    otherwise
    ...
    if ~isfield(features
    {n}.fparams, 'useForColor')
    features
    {n}.fparams.useForColor = true
    end

    if ~isfield(features
    {n}.fparams, 'useForGray')
    features
    {n}.fparams.useForGray = true
    end

    if (features
    {n}.fparams.useForColor & & colorImage) | | (features
    {n}.fparams.useForGray & & ~colorImage)
    ...
    the
    indices
    for the real, positive and negative parts of the
    # spectrum
    ...
    abs(wrs / reg_scale(1)). ^ ref_window_power + abs(wcs / reg_scale(2)). ^ ref_window_power) + params.reg_window_min
    ...
    reg_window_dft = reg_window_dft_sep(:,:, 1) + 1
    i * reg_window_dft_sep(:,:, 2)
    ...
    the
    indices
    for the symmetric(real) part of the spectrum
    ...
    # indexing
    masks
    for upper and lower triangular part
    ...
    if debug
        ...
        imagesc(fftshift(response(:,:, scale_ind)))colorbar
        axis
        image
    ...
    dimension
    xlf = fft2(xl)
    xlf_reshaped = reshape(xlf, [support_sz, feature_dim]) # D_k
    ...
    xlf_reshaped_sym = xlf_reshaped(dft_sym_ind,:) # extract
    the
    symmetric
    part
    of
    ...
    spectrum
    x_0
    xlf_reshaped_pos = xlf_reshaped(dft_pos_ind,:) # extract
    the
    positive
    part
    of
    ...
    if frame == 1
        ...
        for constant regularization
            ...
            filters
            for each feature
                A_init = real(dfs_matrix * spdiags(hf_init_autocorr, 0, support_sz, support_sz) * dfs_matrix
                ') + feature_dim * WW_block
                ...
                figure(99)
                plot(hf_difference)
                axis([1, num_GS_iter * frame, 0, max(hf_difference)])
                ...
                hold off
                axis offaxis imageset(gca, 'Units', 'normalized', 'Position', [0 0 1 1])
                else
                ...
                hold
                on
                resp_handle = imagesc(xs, ys, fftshift(response(:,:, sc_ind))) colormap
                hsv
